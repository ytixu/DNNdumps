...
def plot_fk_from_euler(euler_angles, title='poses', image_dir='../new_out/'):
	...
	for i in range(n):
		rev_coord = revert_coordinate_space( euler_angles[i], np.eye(3), np.zeros(3) )
		for j in range(t):
			xyz_coord = fkl( rev_coord[j], parent, offset, rotInd, expmapInd )
			xyz_coord = np.reshape(xyz_coord, (-1, 3))
			xyz[i][j] = np.copy((xyz_coord[relevant_coords]).flatten())
