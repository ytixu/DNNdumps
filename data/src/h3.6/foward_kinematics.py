...
def plot_poses(expmap_gt, expmap_pred, title, args):
  # Load all the data
  ...
  hierarchies = int(nframes_pred/nframes_gt)
  xyz_pred = np.reshape(xyz_pred, (hierarchies, nframes_gt,  -1))
  n = 5
  skip_t = int(nframes_gt/n)
  f, axarr = plt.subplots(hierarchies+1, n, sharex=True, sharey=True)
  for i in range(n):
    viz.plot_2D_coords(axarr[0, i], xyz_gt[i*skip_t], 3)
    
    for j in range(hierarchies):
      viz.plot_2D_coords(axarr[j+1, i], xyz_pred[j, i*skip_t], 2)

  f.subplots_adjust(hspace=0.1)
  # plt.suptitle(title)
  # plt.show()
  f.savefig(title.lower().replace(' ', '_') + '_'.join(map(str, args)) + strftime("%a-%d-%b-%Y-%H_%M_%S", gmtime()) + '.png') 
  plt.close(f)


def plot(expmap_gt, expmap_pred):
